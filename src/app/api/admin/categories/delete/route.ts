...
export const PUT = async function (req: NextRequest): Promise<NextResponse> {
    try {
        ...
        if (deleteType === "SD") {
            await CategoryModel.updateMany<ICategory>({ _id: { $in: selectedIdList } }, { $set: { deletedAt: new Date().toISOString() } });
        } else {
            await CategoryModel.updateMany<ICategory>({ _id: { $in: selectedIdList } }, { $set: { deletedAt: null } });
        }
