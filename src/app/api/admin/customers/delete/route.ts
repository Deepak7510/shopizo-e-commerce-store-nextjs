...
export const PUT = async function (request: NextRequest): Promise<NextResponse> {
    try {
        ...
        if (deleteType === "SD") {
            await User.updateMany<IUser>({ _id: { $in: selectedIdList } }, { $set: { deletedAt: new Date().toISOString() } });
        } else {
            await User.updateMany<IUser>({ _id: { $in: selectedIdList } }, { $set: { deletedAt: null } });
        }
...
export const DELETE = async function (request: NextRequest): Promise<NextResponse> {
    try {
        ...
        await User.deleteMany({ _id: { $in: selectedIdList } });
