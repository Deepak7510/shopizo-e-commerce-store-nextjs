...
export const PUT = async function (request: NextRequest): Promise<NextResponse> {
    try {
        ...
        if (deleteType === "SD") {
            await CouponModel.updateMany<ICoupon>({ _id: { $in: selectedIdList } }, { $set: { deletedAt: new Date().toISOString() } });
        } else {
            await CouponModel.updateMany<ICoupon>({ _id: { $in: selectedIdList } }, { $set: { deletedAt: null } });
        }

        const message = deleteType === "SD"
            ? "Coupon moved to trash successfully."
...
export const DELETE = async function (request: NextRequest): Promise<NextResponse> {
    try {
        ...
        await CouponModel.deleteMany({ _id: { $in: selectedIdList } });
