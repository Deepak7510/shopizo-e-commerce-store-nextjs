...
import { forgetVerifyOtpService } from "@/services/auth/forgetVerifyOtpService"
...
const ForgetPasswordVerifyOtpPage = ({ params }: { params: Promise<{ email: string }> }) => {
    ...
    async function onSubmit(value: z.infer<typeof optValidationSchema>) {
        const result = await forgetVerifyOtpService({ email, otp: value.otp });
        if (result.success) {
            toast.success(result?.message);
            router.push(authRoutes.resetPassword(result?.data?.token));
        } else {
