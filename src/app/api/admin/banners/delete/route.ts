...
export const PUT = async function (request: NextRequest): Promise<NextResponse> {
    try {
        ...
        if (deleteType === "SD") {
            await BannerModel.updateMany<IBanner>({ _id: { $in: selectedIdList } }, { $set: { deletedAt: new Date().toISOString() } });
        } else {
            await BannerModel.updateMany<IBanner>({ _id: { $in: selectedIdList } }, { $set: { deletedAt: null } });
        }
...
export const DELETE = async function (request: NextRequest): Promise<NextResponse> {
    try {
        ...
        await BannerModel.deleteMany({ _id: { $in: selectedIdList } });
